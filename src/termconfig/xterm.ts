import { EscapeSequence } from '../interface/EscapeSequence';
...
const esc: {[name: string]: EscapeSequence} = {
	...
	column: { on: '\x1b[%UG' },
	row: { on: '\x1b[%Ud' },
	moveTo: { on: '\x1b[%+1U;%-1UH', optimized: (x: string, y: string) => '\x1b[' + y + ';' + x + 'H' },
	...
	backDelete: { on: '\x1b[1D\x1b[1P' },	// Backspace-like, left(1) followed by delete(1) /!\ WARNING /!\ this doesn't
										  // work with full-width char !!! /!\
	...
	bold: { on: '\x1b[1m', off: '\x1b[22m' },		// here we use the dim.off code (22) that have a better support than
											   // (21), for God-only known reason...
	...
	color256: {
		...
		off: defaultColor,
		fb: true,
		handler: function(this: Terminal, register: number) {
	...
	setCursorColor: {
		...
		handler: function(this: Terminal, bg: number, fg: number) {
...
const keymap = {
	...
	SHIFT_TAB: '\x1b[Z',
	ALT_TAB: '\x1b\x09',	// Also CTRL_ALT_I, most of time it is grabbed by the window manager before reaching the
						 // terminal
	ALT_ENTER: '\x1b\x0d',
