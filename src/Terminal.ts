import { GrabInputOptions }        from 'src/interface/GrabInputOptions';
import { YesOrNoOptions }          from 'src/modules/YesOrNoOptions';
import { TerminalKit }             from './TerminalKit';
...
export class Terminal {
	...
	mouseGrabbing = false;
	focusGrabbing = false;
	timeout: number;		// The value is a bit high, to prevent lag due to huge terminal load
	shutdown = false;
	raw = this.stdout.write.bind(this.stdout);	// Used by ScreenBuffer, for optimization

	prependStdinChunk = null;
	...
	constructor(createOptions?: TerminalCreateOptions) {
		if (!createOptions || typeof createOptions !== 'object') {
			createOptions = {};
		}

		this.stdin = createOptions?.stdin ?? process.stdin;
		this.stdout = createOptions?.stdout ?? process.stdout;
		this.stderr = createOptions?.stderr ?? process.stderr;
		this.generic = createOptions?.generic ?? 'xterm';
		...
		this.appId = createOptions?.appId ?? '';
		this.appName = createOptions?.appName ?? '';
		this.isTTY = !!createOptions?.isTTY;
		this.isSSH = !!createOptions?.isSSH;
		this.pid = createOptions?.pid ?? '';

		// The value is a bit high, to prevent lag due to huge terminal load
		this.timeout = this.isSSH ? 500 : 200;
		this.raw = this.stdout.write.bind(this.stdout);	// Used by ScreenBuffer, for optimization

		this.onResize();

		// Resizing event, by order of preference
		if (createOptions.preferProcessSigwinch) {
			process.on('SIGWINCH', this.onResize.bind(this));
		}
		...
		if (this.appId) {
			...
			try {
				this.termconfigFile = this.appId + '.js';
				termconfig = require('./termconfig/' + this.termconfigFile);
			}
			catch (error) {
			} // Do nothing, let the next if block handle the case
		}
		...
		if (!termconfig) {
			...
			try {
				...
				try {
					...
					this.termconfigFile = term.generic + '.js';
					termconfig = require('./termconfig/' + term.termconfigFile);
				}
				catch (error_) {
					// Nothing found, fallback to the most common terminal generic
					this.termconfigFile = 'xterm.generic.js';
					termconfig = require('./termconfig/' + term.termconfigFile);
				}
			}
		}
		...
		this.esc = termconfig.esc; //tree.extend({ deep: true }, {}, termconfig.esc);
		this.support = termconfig.support; //tree.extend({ deep: true }, {}, termconfig.support);
		...
		Object.keys(this.keymap).forEach((key) => {
			...
			let i, j, keymapObject, code, codeList = this.keymap[key];

			if (!Array.isArray(codeList)) {
				codeList = [codeList];
				this.keymap[key] = codeList;
			}
		...
		Object.keys(this.esc).forEach((key) => {
			...
			if (!this.esc[key] || typeof this.esc[key] !== 'object') {
				console.error(
					`Bad escape sequence entry '${key}' using termconfig: '${this.termconfigFile}'.`
				);
				return;
			}
		...
		// Should be done at the end, once everything is working: it needs a configured terminal to generate escape
		// sequences
	...
	protected onStdin(chunk) {
		...
		while (index < length) {
			...
			if (chunk[index] <= 0x1f || chunk[index] === 0x7f) {
				...
				if (chunk[index] < 0xc0) {
					continue;
				}	// We are in a middle of an unicode multibyte sequence... Something fails somewhere, we will just
					 // continue for now...
	 ...
	 *
	 */
	async grabInput(options?: GrabInputOptions, safe: boolean = false) {
		// RESET
		...
		if (options === false) {
			if (safe) {
				await setTimeout(()=> {}, this.timeout / 2);
				disable();
				return;
			}
	...
	yesOrNo(options: Function|YesOrNoOptions, callback: Function) {
	...
	protected colorRgbHandler(r: number|string, g: number, b: number) {
		let c, rgb;

		if (typeof r === 'string') {
			rgb = TerminalKit.hexToRgba(r);
			r = rgb.r;
			g = rgb.g;
			b = rgb.b;
		}
		...
		if (
			isNaN(r as number) || isNaN(g) || isNaN(b) ||
	...
	colorNameForHex(hex: string) {
		const rgba = TerminalKit.hexToRgba(hex);
...
function applyEscape(options, ...args) {
	...
	if (options.forceStyleOnReset) {
		action = action.replace(new RegExp(string.escape.regExp(options.root.optimized.styleReset), 'g'),
